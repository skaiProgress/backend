...
func (r *postgresRepository) List(ctx context.Context, search string) ([]CourseWithCount, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&item.CreatedBy, &item.CreatedAt, &item.UpdatedAt, &item.LessonCount,
		); err != nil {
			return nil, err
		}
		out = append(out, item)
...
func (r *postgresRepository) GetByID(ctx context.Context, id string) (*Course, error) {
	const q = `
		SELECT id::text, title, description, status, cover_url, created_by::text, created_at, updated_at
...
func (r *postgresRepository) Create(ctx context.Context, c Course) (*Course, error) {
	const q = `
		INSERT INTO public.courses (title, description, status, cover_url, created_by)
		VALUES ($1, $2, $3, $4, NULLIF($5, '')::uuid)
		RETURNING id::text, title, description, status, cover_url, created_by::text, created_at, updated_at
	`
	createdBy := ""
	if c.CreatedBy != nil {
	...
	var out Course
	err := r.pool.QueryRow(ctx, q, c.Title, c.Description, c.Status, c.CoverURL, createdBy).Scan(
